...
sys.path.append('..')
import utils
from models import FuzzyDG, DFGN
...
if __name__ == '__main__':
    ...
    if target_domain in nli_domains:
        if target_domain == 'sick' or target_domain == 'snli':
            backbone = FuzzyDG.ClassificationHeadModel(model_path, s_num_labels=3, d_num_labels=5).to(device)
        else:
            backbone = FuzzyDG.ClassificationHeadModel(model_path, s_num_labels=3, d_num_labels=4).to(device)
        cl = DFGN.CenterLoss(3, 768).to(device)
    elif target_domain in sa_domains:
        if target_domain == 'imdb' or target_domain == 'sst':
            backbone = FuzzyDG.ClassificationHeadModel(model_path, s_num_labels=2, d_num_labels=4).to(device)
        else:
            backbone = FuzzyDG.ClassificationHeadModel(model_path, s_num_labels=2, d_num_labels=3).to(device)
        cl = DFGN.CenterLoss(2, 768).to(device)
    else:
        backbone = FuzzyDG.ClassificationHeadModel(model_path, s_num_labels=2, d_num_labels=4).to(device)
        cl = DFGN.CenterLoss(2, 768).to(device)

    er = DFGN.EntropyRegularization(lambda_param=1.0).to(device)

    mean_sort = 'full'
    mean_decay_param = 1.0
    cov_weighting_loss = FuzzyDG.CoVWeightingLoss(mean_sort, mean_decay_param, device, num_losses=3)
    ...
    for epoch in range(num_epochs):
        ...
        for step, batches in enumerate(pbar):
            ...
            # Here, it can be guaranteed that the data added each time is from a different domain,
            # because the data traversed in train_dataloader is from different domains
            for idx, batch in enumerate(batches):
                ...
                s_logits, d_logits, cls_hidden_states = backbone(input_ids, att_masks)
            ...
            hidden_states = torch.cat(cls_hidden_states_list, dim=0)

            labels = torch.cat(labels, dim=0).to(device)
            domain_labels = torch.tensor(domain_label_list).flatten().to(device)

            label_ce_loss = criteria(s_logits, labels)
            domain_ce_loss = criteria(d_logits, domain_labels)
            er_loss = er(hidden_states)
            # cl_loss = cl(hidden_states, labels)

            loss = cov_weighting_loss([label_ce_loss, domain_ce_loss, er_loss])

            # loss = 0.7 * label_ce_loss + 0.1 * domain_ce_loss + 0.1 * er_loss + 0.1 * cl_loss
        ...
        if target_domain == 'imdb' or target_domain == 'sst' or target_domain == 'sick' or target_domain == 'snli':
            val_loss, f1 = evaluate(backbone, val_dataloader, target_domain)

            if val_loss < best_val_loss:
                best_val_loss = val_loss
                # torch.save(model.state_dict(), save_dir)

                _, test_f1 = evaluate(backbone, test_dataloader, target_domain)
            ...
            _, f1 = evaluate(backbone, test_dataloader, target_domain)
